...
def download_lastest_chromedriver(current_chrome_version=""):
    ...
    try:
        ...
        if all_match:
            ...
            chromedriver_path = os.path.normpath(os.path.join(app_path, 'Google-Image-Scraper-master/webdriver', webdriver_executable()))
            file_path = os.path.normpath(os.path.join(app_path, 'Google-Image-Scraper-master/webdriver', file_name))
            urllib.request.urlretrieve(driver_url, file_path)
    
            # Unzip the file into folder
            with zipfile.ZipFile(file_path, 'r') as zip_ref:
                zip_ref.extractall(os.path.normpath(os.path.join(app_path, 'Google-Image-Scraper-master/webdriver')))
